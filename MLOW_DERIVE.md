...
- `crates/oracle-core/src/derive.rs` (novo): schema da spec, resolvedor,
  executor, `manifest.json` (sem timestamps: mesmos bytes + mesma spec =
  mesmos bytes de saída).
- `crates/oracle-core/src/abi.rs`: `table_slots_of` (slot→func inverso),
  `function_count`.
- `crates/oracle-cli/src/main.rs`: subcomando `Derive`.
- `README-oracle.md`: uma linha no índice de comandos.
...
18. **Vetor 3-frame travado**: op `read` ganhou `Len` (`$reg` ou literal;
    ...
    antes; `specs/mlow_3frames.json` = open + 3×(encode→`read_reg`→decode)
    + `assert_sha256` nos 6 outputs. Pacotes 137/139/138 B, PCM
    1.147/1.260/1.158×, manifest byte-idêntico entre runs.
19. **Vetor 110 travado** (`specs/mlow_110frames.json`, 2706 steps):
